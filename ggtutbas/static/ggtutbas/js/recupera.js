	...
	$('#recuperar-password').on('click', function(e){
        ...
        $('#forgot-password').html('');
          ...
          if ($('#rut').val().trim()=="" || $('#validador').val().trim()==""){
            if (pais=='cl'){
                $('#errorRespuesta').html('<span>Para recuperar su contraseña ingrese su Rut</span>').fadeIn('fast');
                }
             else{
             $('#errorRespuesta').html('<span>Para recuperar su contraseña ingrese su Id usuario</span>').fadeIn('fast');
             }
          }
          else  if (Fn.validaRut(rutCompleto) == false){
            if (pais=='cl'){
             $('#errorRut').html('<span>El rut ingresado no es válido. Inténtelo nuevamente</span>').fadeIn('fast');
             }
             else{
             $('#errorRut').html('<span>El Id usuario ingresado no es válido. Inténtelo nuevamente</span>').fadeIn('fast');
             }
         }
         else if($('#respuesta-secreta').val().trim()==""){
              $('#errorRespuesta').html('<span>Para recuperar su contraseña ingrese su respuesta a la pregunta de seguridad</span>').fadeIn('fast');
           }
            else{
            cargaAnimacion();
                $.ajax({
                        ...
                        success: function(respuesta){
                            ...
                            if (respuesta.status=='respuesta ok'){
                                ...
                                $('#box-password-recuperada').fadeOut('fast');
                                $('#forgot-password').html('');
                                $('#errorRespuesta').html('<span>La respuesta ingresada no es válida. Inténtelo nuevamente</span>').fadeIn('fast');
                            }
                          },
