...
from ggalbas.models import TblAlumnos, TblListas, TblPreguntausuarios, TblSubproducto, TblRegistroipAlumno, TblNiveles, \
    ...
    TblHabilidades, TblEje, TblAlumnoDiagnostico, TblContenidoUnidad, TblUnidades, TblContenidosFasesActividades, TblContenidos
...
def visorActividades(request):
    ...
    audio_pregunta = 'P' + str(id_nivel) + 'GG01' + str(year) + '_e' + str(npreg)+'_int'
...
def contenidosAlumno(request):
    ...
    listaAct = dict()
    actMinitest = dict()
    actRecuperacion = dict()
    actNivel = dict()

    objContenidos = TblContenidos.objects.get(id_contenido=id_contenido)
    nombre_contenido = objContenidos.descripcion
    nombre_unidad = objContenidos.id_unidad.nombre_unidad
    objContenidosFasesActividades = TblContenidosFasesActividades.objects.filter(id_contenido=objContenidos).order_by('orden')
    if objContenidosFasesActividades:
        for result in objContenidosFasesActividades:
            act = {result.id_actividades.id_actividad: result.id_actividades.descripcion_actividades}
            if result.id_fase.id_fase in listaAct:
                listaAct[result.id_fase.id_fase].update(act)
            else:
                listaAct[result.id_fase.id_fase] = {}
                listaAct[result.id_fase.id_fase].update(act)
    # minitest
    if 2 in listaAct:
        actMinitest = listaAct[2]
    # Recuperacion
    if 3 in listaAct:
        actRecuperacion = listaAct[3]
    # Nivel
    if 4 in listaAct:
        actNivel = listaAct[4]
